...
void nm25q128_init(void){
    ...
    spi2_init();
    spi2_start();
    spi2_swap_byte(NM25Q128_CMD_RELEASE_POWER_DOWN);
    spi2_stop();
    Delay_us(100);

    spi2_start();
    spi2_swap_byte(NM25Q128_CMD_RESET_ENABLE);
    spi2_stop();
    spi2_start();
    spi2_swap_byte(NM25Q128_CMD_RESET_MEMORY);
    spi2_stop();
    Delay_us(100);
}
...
void nm25q128_read_id(uint8_t *mid,uint16_t *did){
    ...
    // 2.1 发送9F指令
    spi2_swap_byte(NM25Q128_CMD_READ_ID);
    // 2.2 接收数据的时候   填写的交换发送数据是没有意义的
    *mid = spi2_swap_byte(0XFF);
    *did = 0;
    *did |= spi2_swap_byte(0XFF) << 8;
    *did |= spi2_swap_byte(0XFF) << 0;
...
void nm25q128_wait_busy(void){
    ...
    spi2_start();
    // 2. 发送读取状态指令
    spi2_swap_byte(NM25Q128_CMD_READ_STATUS1);
    // 3. 接收状态寄存器的值
    status = spi2_swap_byte(0xff);
    // 如果最低位为1  表示忙
    while (status & 0x01){
        status = spi2_swap_byte(0xff);
    }
...
void nm25q128_write_enable(void){
    ...
    spi2_start();
    // 2. 开启写使能
    spi2_swap_byte(NM25Q128_CMD_WRITE_ENABLE);
    // 3. 片选关闭
    spi2_stop();
}
...
void nm25q128_write_disenable(void){
    ...
    spi2_start();
    // 2. 关闭写使能
    spi2_swap_byte(NM25Q128_CMD_WRITE_DISABLE);
    // 3. 片选关闭
    spi2_stop();
}
...
void nm25q128_write_page(uint8_t block,uint8_t sector,uint8_t page,uint8_t data[],uint8_t dataLen){
    ...
    spi2_start();
    // 3.2 发送写数据的指令
    spi2_swap_byte(NM25Q128_CMD_PAGE_PROGRAM);
    // 3.3 写入数据的起始地址
    // W25Q32的地址是22位   表示的时候使用24位  =  6个16进制表示
    uint32_t addr = block * 0x010000 + sector * 0x001000 + page * 0x000100;
    spi2_swap_byte(addr>>16);
    spi2_swap_byte((addr >> 8) & 0xff);
    spi2_swap_byte((addr >> 0) & 0xff);
    // 3.4 写入数据
    for (uint8_t i = 0; i < dataLen; i++){
        spi2_swap_byte(data[i]);
    }
...
void nm25q128_sector_erase(uint8_t block,uint8_t secotr){
    ...
    spi2_start();
    // 3.2 发送擦除命令
    spi2_swap_byte(NM25Q128_CMD_SECTOR_ERASE);
    // 3.3 发送擦除地址
...
void nm25q128_read_data(uint8_t block,uint8_t sector,uint8_t page,uint8_t buff[],uint16_t dataLen){
    ...
    spi2_start();
    // 2.2 发送读数据指令
    spi2_swap_byte(NM25Q128_CMD_READ_DATA);
    uint32_t addr = block * 0x010000 + sector * 0x001000 + page * 0x000100;
    spi2_swap_byte(addr>>16);
    spi2_swap_byte((addr >> 8) & 0xff);
    spi2_swap_byte((addr >> 0) & 0xff);
