...
const uint8_t udp6_all_nodes[16] = {
    0xff, 0xff,
    0xff, 0xff,
    0xff, 0xff,
    0xff, 0xff,
...
void InitSocketParam(void)
{
    ...
    memcpy(SockInf[0].IPv6Addr, udp6_all_nodes, sizeof(udp6_all_nodes)); /* Write destination IP address */
    SockInf[0].DesPort = Socket0DesPort;                                 /* Destination port */
    SockInf[0].SourPort = Socket0SourPort;                               /* Source port */
    SockInf[0].ProtoType = PROTO_TYPE_UDP6;                              /* UDP mode */
}
...
void CH395SocketInitOpen(void)
{
    ...
    CH395CMDSetDesIPv6Addr(0, SockInf[0].IPv6Addr);     /* Set destination IP address for socket 0 */
...
/**********************************************************************************
 * Function Name  : UDP6_MUlticast_Data_Loop
 * Description    : Receive data from socket
 * Input          : socket index
 * Output         : None
 * Return         : None
 * *********************************************************************************/
void UDP6_MUlticast_Data_Loop(UINT8 sockindex)
{
    u16 port;
    if (Recv_len[sockindex] == 0)
    {
        if (sock_int_status[sockindex] & SINT_STAT_RECV)
        {
            ...
            printf("IPv6 = %02X%02X:%02X%02X:%02X%02X:%02X%02X:"
                   ...
                   MyBuffer[sockindex][4], MyBuffer[sockindex][5],
                   MyBuffer[sockindex][6], MyBuffer[sockindex][7],
                   MyBuffer[sockindex][8], MyBuffer[sockindex][9],
                   MyBuffer[sockindex][10], MyBuffer[sockindex][11],
                   MyBuffer[sockindex][12], MyBuffer[sockindex][13],
                   MyBuffer[sockindex][14], MyBuffer[sockindex][15],
                   MyBuffer[sockindex][16], MyBuffer[sockindex][17],
                   MyBuffer[sockindex][18], MyBuffer[sockindex][19]);
        }
    }
    else
    {
        if (sock_int_status[sockindex] & SINT_STAT_SENDBUF_FREE)
        {
            ...
            port = ((UINT16)MyBuffer[sockindex][3] << 8) + MyBuffer[sockindex][2];
            CH395UDP6SendTo(&MyBuffer[sockindex][20], (Recv_len[sockindex] - 20), &MyBuffer[sockindex][4], port, sockindex);
...
void CH395GlobalInterrupt(void)
{
    ...
    {
#if CH395_DHCP
        i = CH395CMDGetDHCPv6Status();

        if (i == 0)
        {
            UINT8 MyIPv6Addr[32];
            flag = 1;
            CH395CMDGetIPv6Addr(MyIPv6Addr);
